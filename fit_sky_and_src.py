...
sky_model = (
    sky_scale * (LHB + SkyAbs * (MWhalo + CXB)) + InstLine_1 + InstLine_2 + SoftProton
)
...
for i, bid in enumerate(['BKG_1', 'BKG_2']):
    ...
    ax_main.step(
        ...
        color='orange',
        lw=1.8,
        label='Total Model' if i == 0 else None,
    )
...
def fit_and_plot_source():
    ...
    SrcNEI1.kT.freeze()
    SrcNEI1.Tau.set(val=6e11, min=1e11, max=4e13)
    for par_name in ['Mg', 'Si', 'S', 'Ar', 'Ca']:
    ...
    _fit_stage(
        "Stage 1: thaw nH, SrcNEI1 (kT, Tau, Si, S), SrcNEI2 (kT, Tau)", stage1_params
    )
